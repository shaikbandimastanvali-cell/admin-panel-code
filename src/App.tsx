...
const supabaseUrl = 'https://anodayscpvhbvsbxecmf.supabase.co';
...
isSupported().then(supported => { if (supported) getMessaging(app); }).catch(e => console.warn("FCM check failed:", e));

// --- HELPER FUNCTIONS ---
const generateId = () => Math.random().toString(36).substr(2, 9) + Date.now().toString(36);

// Supabase Error Catcher (Throws errors so the Admin Modal catches them)
const runDb = async (request) => {
...
const sendPushNotification = async ({ title, body, targetUids, data = {}, users }) => {
  try {
    ...
    if (fcmTokens.length === 0) return; // No tokens to send to
    ...
    // We try to send it, but if Vercel rejects it, we just log a warning instead of crashing!
    ...
    if (!res.ok) console.warn(`Push API 400: Your Vercel backend needs configuration. Push skipped.`);
  } catch(e) {
    console.warn("Push notification failed safely.", e);
  }
};
...
// 🔥 ADMIN REALTIME: Instant UI Updates 🔥
  const [trigger, setTrigger] = useState(0);

  useEffect(() => {
    const channel = supabase.channel('admin-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'users' }, () => setTrigger(prev => prev + 1))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'transactions' }, () => setTrigger(prev => prev + 1))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'tournaments' }, () => setTrigger(prev => prev + 1))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'messages' }, () => setTrigger(prev => prev + 1))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'settings' }, () => setTrigger(prev => prev + 1));
    
    channel.subscribe();
    return () => { supabase.removeChannel(channel); };
  }, []);

  // Fetch Data Instantly when triggered
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        const [u, g, m, t, tx, bd, msg, s] = await Promise.all([
          supabase.from('users').select('*'),
          supabase.from('games').select('*'),
          supabase.from('modes').select('*'),
          supabase.from('tournaments').select('*'),
          supabase.from('transactions').select('*'),
          supabase.from('banneddevices').select('*'),
          supabase.from('messages').select('*'),
          supabase.from('settings').select('*').eq('id', 'global').single()
        ]);
        setData({ users: u.data||[], games: g.data||[], modes: m.data||[], tournaments: t.data||[], transactions: tx.data||[], bannedDevices: bd.data||[], messages: msg.data||[] });
        if (s.data && s.data.data) setSettings(p => ({...p, ...s.data.data}));
      } catch(e) { console.error("Admin Fetch Error:", e); }
    };
    
    fetchAdminData();
    // Safety fallback: only polls every 10 minutes to save server CPU, relies entirely on WebSockets!
    const interval = setInterval(fetchAdminData, 6000); 
    return () => clearInterval(interval);
  }, [trigger]); // 🔥 THE WIRE: Refreshes instantly when any table changes
  ...
  useEffect(() => {
    ...
    if (data.users && data.users.length === 0 && !loading) { 
      runDb(supabase.from('users').insert({ uid: "root-1", name: 'Super Admin', email: 'qwerty@qwerty.com', password: 'zxcvbnm', role: 'admin', balance: 0, depositBalance: 0, winningBalance: 0, totalWinnings: 0, totalKills: 0, joinedDate: new Date().toISOString(), isBanned: false, permissions: [] })).then(()=>{}); 
      runDb(supabase.from('publicprofiles').insert({ uid: "root-1", name: 'Super Admin' })).then(()=>{});
    }
    setLoading(false);
  }, [data.users]);
  ...
  const login = async (e) => {
    ...
    const { data: users, error } = await supabase.from('users').select('*').eq('email', em).eq('password', pw);
    if(users && users.length > 0) {
      const u = users[0];
      if(u.role==='admin'||u.role==='staff') { localStorage.setItem('admin_uid', u.uid); setAdmin(u); } else alert("Access Denied/Invalid");
  ...
  if (su) {
    ...
    return (
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        </div><div className="mt-4 flex gap-2"><button onClick={async ()=>{ try { await runDb(supabase.from('users').update({isBanned:!su.isBanned}).eq('uid', su.uid)); await runDb(supabase.from('publicprofiles').update({isBanned:!su.isBanned}).eq('uid', su.uid)); md({t:'alert',title:'Success',msg:`User ${su.isBanned ? 'unbanned' : 'banned'} successfully`}); } catch(err){ md({t:'err',title:'Error',msg:err.message}); } }} className={`flex-1 py-3 rounded-lg font-black uppercase text-xs text-white cursor-pointer ${su.isBanned?'bg-emerald-500':'bg-rose-500'}`}>{su.isBanned?'Unban':'Ban'} User</button>
        {adminUser.role === 'admin' && <button onClick={()=>md({t:'confirm', title:'Delete User', msg:'Permanently delete this user?', onC:async ()=>{ await runDb(supabase.from('users').delete().eq('uid', su.uid)); await runDb(supabase.from('publicprofiles').delete().eq('uid', su.uid)); setSu(null);}})} className="flex-1 py-3 rounded-lg font-black uppercase text-xs bg-rose-100 text-rose-700 cursor-pointer">Delete</button>}
        </div>
        <button onClick={async ()=>{ if(!su.deviceId) return md({t:'err',title:'Error',msg:"No device ID recorded for this user yet."}); try { await runDb(supabase.from('banneddevices').insert({id: generateId(), deviceId: su.deviceId, bannedAt: new Date().toISOString()})); md({t:'alert',title:'Success',msg:'Hardware Device Banned Successfully'}); } catch(err){ md({t:'err',title:'Error',msg:err.message}); } }} className="w-full mt-2 py-3 rounded-lg font-black uppercase text-xs bg-slate-900 text-white cursor-pointer hover:bg-slate-800 transition-colors">Ban Hardware Device</button>
        </div>
        <div><div className="flex justify-between items-center mb-4"><h3 className="font-black text-xl">Stats</h3><button onClick={()=>md({t:'estats',title:'Edit Stats',d1:su.balance,d2:su.totalWinnings,d3:su.totalKills,d4:su.totalRefers,d5:su.winningBalance,d6:su.totalMatches,d7:su.depositBalance,onC:async (b,w,k,r,wb,tm,td)=>{ try { await runDb(supabase.from('users').update({balance:Number(b),winningBalance:Number(wb),totalWinnings:Number(w),totalKills:Number(k),totalRefers:Number(r),totalMatches:Number(tm),depositBalance:Number(td)}).eq('uid', su.uid)); await runDb(supabase.from('publicprofiles').update({totalWinnings:Number(w),totalKills:Number(k)}).eq('uid', su.uid)); setSu({...su,balance:Number(b),winningBalance:Number(wb),totalWinnings:Number(w),totalKills:Number(k),totalRefers:Number(r),totalMatches:Number(tm),depositBalance:Number(td)}); } catch(err){ md({t:'err',title:'Error',msg:err.message}); } }})} className="bg-slate-900 text-white px-3 py-1.5 rounded text-xs font-bold uppercase cursor-pointer"><Edit className="w-3 h-3 inline"/> Edit</button></div>
            ...
            <div key={m.id} className="p-5 hover:bg-slate-50 transition-colors flex justify-between items-start gap-4">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => md({t:'message', title:'Edit Message', d1: m.title, d2: m.body, ul: d.users, onC: async (title, body) => {
                  if(!title || !body) return;
                  try { await runDb(supabase.from('messages').update({ title, body }).eq('id', m.id)); md({t:'alert', title:'Success', msg:'Message updated successfully'}); } catch(e) { md({t:'err', title:'Error', msg:e.message}); }
                ...
                {adminUser.role === 'admin' && <button onClick={() => md({t:'confirm', title:'Delete Message', msg:'Permanently delete this message for all users?', onC: async () => {
                  try { await runDb(supabase.from('messages').delete().eq('id', m.id)); md({t:'alert', title:'Success', msg:'Message deleted'}); } catch(e) { md({t:'err', title:'Error', msg:e.message}); }
...
function GamesMgr({ d, md, u }) {
  return <div className="space-y-6"><div className="flex justify-between items-center bg-white p-6 rounded-2xl border"><h2 className="font-black text-xl uppercase">Games & Modes</h2><div className="flex gap-2">
    <button onClick={()=>md({t:'mode',title:'Add Mode',gl:d.games,onC:async(g,n,b,p)=>{if(g&&n) await runDb(supabase.from('modes').insert({id: generateId(), gameId:g,name:n,bannerUrl:b,priority:Number(p||0),createdBy:u.name}));}})} className="bg-indigo-100 text-indigo-700 px-4 py-2 rounded-lg font-black text-xs uppercase cursor-pointer">Add Mode</button><button onClick={()=>md({t:'game',title:'Add Game',onC:async(n,b)=>{if(n) await runDb(supabase.from('games').insert({id: generateId(), name:n,bannerUrl:b,createdBy:u.name}));}})} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-black text-xs uppercase cursor-pointer">Add Game</button></div></div><div className="grid md:grid-cols-2 gap-6">{d.games.map(g=><div key={g.id} className="bg-white rounded-2xl border overflow-hidden"><div className="h-32 bg-slate-900 relative">{g.bannerUrl?<img src={g.bannerUrl} className="w-full h-full object-cover opacity-60"/>:<Gamepad2 className="m-auto mt-8 text-white/20 w-12 h-12"/>}<div className="absolute bottom-2 left-4 text-white font-black text-2xl uppercase">{g.name}</div><div className="absolute top-2 left-2 bg-black/60 px-2 py-1 rounded text-[8px] font-bold text-white uppercase">By: {g.createdBy||'Admin'}</div>
    {u.role === 'admin' && <button onClick={()=>{const hasModes = d.modes.some(m => m.gameId === g.id); if (hasModes) return md({t: 'err', title: 'Cannot Delete', msg: 'Please delete all modes inside this game first.'}); md({t: 'confirm', title: 'Delete Game', msg: `Are you sure you want to delete ${g.name}?`, onC: () => runDb(supabase.from('games').delete().eq('id', g.id))})}} className="absolute top-2 right-2 p-1.5 bg-rose-500 rounded text-white cursor-pointer hover:bg-rose-600 transition-colors"><Trash2 className="w-4 h-4"/></button>}
    ...
    <div className="flex gap-2">
      {u.role === 'admin' && <button onClick={()=>md({t:'mode', title:'Edit Mode', gl:d.games, d1:m.gameId, d2:m.name, d3:m.bannerUrl, d4:m.priority, onC:(ga,na,ba,pr)=>{runDb(supabase.from('modes').update({gameId:ga, name:na, bannerUrl:ba, priority:Number(pr||0)}).eq('id', m.id));}})} className="text-blue-500 cursor-pointer p-2 hover:bg-blue-50 rounded"><Edit className="w-4 h-4"/></button>}
      {u.role === 'admin' && <button onClick={()=>md({t: 'confirm', title: 'Delete Mode', msg: `Delete ${m.name}?`, onC: () => runDb(supabase.from('modes').delete().eq('id', m.id))})} className="text-rose-500 cursor-pointer p-2 hover:bg-rose-50 rounded"><Trash2 className="w-4 h-4"/></button>}
    </div>
    </div>)}</div></div>)}</div></div>;
}

// 🔥 TOURNEY MGR: BLUE FRIENDS, PAYOUT ROUTING, & DOUBLE-CLICK FLAGS 🔥
function TourneyMgr({ d, md, u, s }) {
  const [fo, setFo] = useState(false); 
  const [fd, setFd] = useState({gameId:'',modeId:'',title:'',bannerUrl:'',customText:'',dateTime:'',type:'solo',perKill:0,entryFee:0,totalSlots:48,status:'upcoming'}); 
  const [tb, setTb] = useState('active');
  const [isPublishing, setIsPublishing] = useState(false); 
  const [editId, setEditId] = useState(null);
  
  const [expG, setExpG] = useState(null); 
  const [expM, setExpM] = useState(null);
  const [flagUid, setFlagUid] = useState(null); // 🔥 State for Double-Click Modal
  ...
  const sv = async (e) => {
    ...
    try {
      if (editId) {
        await runDb(supabase.from('tournaments').update({...fd}).eq('id', editId));
        setEditId(null);
      } else {
        await runDb(supabase.from('tournaments').insert({id: generateId(), ...fd, joinedUsers: [], results: [], createdBy: u.name}));
      }
      setFo(false); md({t:'alert',title:'Success',msg:'Updated successfully'});
        ...
        onC:async ()=>{
          ...
          try {
            ...
            await Promise.all((t.joinedUsers||[]).map(async ju=>{
              // 🔥 Route refund to sponsor if it's a friend
              const targetUid = ju.isFriend ? ju.sponsorUid : (ju.uid || ju);
              const uu = d.users.find(x=>x.uid===targetUid);
              if(uu && fee > 0) {
                await runDb(supabase.from('users').update({ balance: Number(uu.balance||0)+fee, depositBalance: Number(uu.depositBalance||0)+fee }).eq('uid', targetUid));
                await runDb(supabase.from('transactions').insert({id: generateId(), uid: targetUid, type:'refund',amount:fee,description:`Refund: ${t.title} ${ju.isFriend?'(Sponsored Friend)':''}`,status:'completed',date:new Date().toISOString()}));
              }
            }));
            await runDb(supabase.from('tournaments').update({status:ns}).eq('id', t.id));
            md({t:'alert',title:'Success',msg:'Match cancelled and 100% of users refunded!'});
          } catch (err) { md({t:'err',title:'Error',msg:err.message}); }
        }
      });
    } else {
      try { runDb(supabase.from('tournaments').update({status:ns}).eq('id', t.id)); md({t:'alert',title:'Success',msg:'Updated successfully'}); } 
      catch(err) { md({t:'err',title:'Error',msg:err.message}); }
    }
  };
  ...
  const oR = (t) => { 
     const o={}; 
     (t.joinedUsers||[]).forEach(ju=>{
        const id = ju.uid || ju; 
        const uData = d.users.find(x=>x.uid===(ju.isFriend ? ju.sponsorUid : id));
        const gName = ju.isFriend ? ju.gameName : (uData ? (uData.gameName || uData.name) : 'Player');
        // 🔥 Preserve the friend & sponsor data for the payout routing
        o[id] = { gameName: gName, kills: 0, earned: 0, uid: id, isFriend: ju.isFriend, sponsorUid: ju.sponsorUid };
     }); 
  ...
  const sR = () => {
    ...
    md({
      ...
      onC:async ()=>{
        ...
        try {
          await runDb(supabase.from('tournaments').update({status:'result',results:a,customText:rn}).eq('id', rt.id));
          await Promise.all(a.map(async r=>{
            if(r.earned>0||r.kills>0){
              // 🔥 CRITICAL FIX: Route money and kills directly to the Sponsor!
              const targetUid = r.isFriend ? r.sponsorUid : r.uid;
              const xu=d.users.find(x=>x.uid===targetUid);
              if(xu) {
                 const up={};
                 if(r.earned>0){ 
                   ...
                   await runDb(supabase.from('transactions').insert({id: generateId(), uid:targetUid,type:'winnings',amount:r.earned,description:`Won: ${rt.title} ${r.isFriend ? `(via ${r.gameName})` : ''}`,status:'completed',date:new Date().toISOString()})); 
                 }
                 if(r.kills>0) up.totalKills=Number(xu.totalKills||0)+Number(r.kills);
                 await runDb(supabase.from('users').update(up).eq('uid', targetUid));
              }
            }
          }));
          setRt(null); md({t:'alert',title:'Success',msg:'Results Published! Money routed to sponsors.'});
        } catch(err) { md({t:'err',title:'Error',msg:err.message}); } finally { setIsPublishing(false); }
      }
    });
  };
  ...
  const groupedByGame = {};
  filteredT.forEach(t => { if(!groupedByGame[t.gameId]) groupedByGame[t.gameId] = {}; if(!groupedByGame[t.gameId][t.modeId]) groupedByGame[t.gameId][t.modeId] = []; groupedByGame[t.gameId][t.modeId].push(t); });

  return <div className="space-y-6 relative">
    
    {/* 🔥 THE DOUBLE-CLICK FLAG MODAL 🔥 */}
    {flagUid && (
      <div className="fixed inset-0 bg-slate-900/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
        <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-xs text-center animate-in zoom-in-95">
          <h3 className="font-black uppercase tracking-widest mb-4 text-slate-800">Flag User</h3>
          <div className="space-y-3">
            <button onClick={async ()=>{ await runDb(supabase.from('users').update({adminFlag:'red'}).eq('uid',flagUid)); setFlagUid(null); md({t:'alert',title:'Flagged',msg:'User marked as Red.'}); }} className="w-full py-4 bg-rose-600 text-white font-black rounded-xl uppercase tracking-widest shadow-md hover:bg-rose-700 cursor-pointer">Red Flag</button>
            <button onClick={async ()=>{ await runDb(supabase.from('users').update({adminFlag:'violet'}).eq('uid',flagUid)); setFlagUid(null); md({t:'alert',title:'Flagged',msg:'User marked as Violet.'}); }} className="w-full py-4 bg-violet-600 text-white font-black rounded-xl uppercase tracking-widest shadow-md hover:bg-violet-700 cursor-pointer">Violet Flag</button>
            <button onClick={async ()=>{ await runDb(supabase.from('users').update({adminFlag:null}).eq('uid',flagUid)); setFlagUid(null); md({t:'alert',title:'Cleared',msg:'Flags removed.'}); }} className="w-full py-4 bg-slate-100 text-slate-600 font-black rounded-xl uppercase tracking-widest hover:bg-slate-200 cursor-pointer">Remove Flag</button>
          </div>
          <button onClick={()=>setFlagUid(null)} className="mt-5 text-xs font-bold text-slate-400 uppercase tracking-widest cursor-pointer">Cancel</button>
        </div>
      </div>
    )}

    <div className="flex justify-between items-center bg-white p-6 rounded-2xl border shadow-sm"><h2 className="text-xl font-black uppercase">Manage Tournaments</h2><button onClick={()=>{setFd({gameId:'',modeId:'',title:'',bannerUrl:'',customText:'',dateTime:'',type:'solo',perKill:0,entryFee:0,totalSlots:48,status:'upcoming'}); setEditId(null); setFo(!fo);}} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-black text-xs uppercase cursor-pointer hover:bg-blue-700"><Plus className="w-4 h-4 inline"/> Create</button></div>
    
    {fo&&<form onSubmit={sv} className="bg-white p-6 rounded-2xl border-2 border-blue-200 grid md:grid-cols-2 gap-4 shadow-sm"><select required value={fd.gameId} onChange={e=>setFd({...fd,gameId:e.target.value,modeId:''})} className="p-3 border rounded-xl font-bold outline-none cursor-pointer"><option value="">Game...</option>{d.games.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select><select required disabled={!fd.gameId} value={fd.modeId} onChange={e=>{ const sm = d.modes.find(m => m.id === e.target.value); setFd(p => ({ ...p, modeId: e.target.value, ...(sm?.preset || {}) })); }} className="p-3 border rounded-xl font-bold outline-none cursor-pointer"><option value="">Mode...</option>{d.modes.filter(m=>m.gameId===fd.gameId).map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select><input required type="text" placeholder="Title" value={fd.title} onChange={e=>setFd({...fd,title:e.target.value})} className="p-3 border rounded-xl font-bold"/><input type="datetime-local" required value={fd.dateTime} onChange={e=>setFd({...fd,dateTime:e.target.value})} className="p-3 border rounded-xl font-bold"/><select required value={fd.type} onChange={e=>setFd({...fd,type:e.target.value})} className="p-3 border rounded-xl font-bold outline-none cursor-pointer"><option value="solo">Solo</option><option value="duo">Duo</option><option value="squad">Squad</option><option value="clash_squad">Clash Squad</option></select><div className="flex gap-2"><input required type="number" placeholder="Fee" value={fd.entryFee} onChange={e=>setFd({...fd,entryFee:Number(e.target.value)})} className="flex-1 p-3 border rounded-xl font-black text-blue-600"/><input required type="number" placeholder="Per Kill" value={fd.perKill} onChange={e=>setFd({...fd,perKill:Number(e.target.value)})} className="flex-1 p-3 border rounded-xl font-black text-emerald-600"/><input required type="number" placeholder="Slots" value={fd.totalSlots} onChange={e=>setFd({...fd,totalSlots:Number(e.target.value)})} className="flex-1 p-3 border rounded-xl font-black"/></div><input type="text" placeholder="Banner URL" value={fd.bannerUrl} onChange={e=>setFd({...fd,bannerUrl:e.target.value})} className="col-span-full p-3 border rounded-xl font-bold"/><textarea value={fd.customText} onChange={e=>setFd({...fd,customText:e.target.value})} placeholder="Custom Instructions/Rules" className="col-span-full p-3 border rounded-xl font-bold outline-none h-24"/><div className="col-span-full flex gap-2 mt-2"><button type="submit" className="flex-1 py-4 bg-blue-600 text-white font-black rounded-xl uppercase cursor-pointer hover:bg-blue-700">{editId ? 'UPDATE MATCH' : 'SAVE MATCH'}</button>{fd.modeId && (<button type="button" onClick={async () => { try { await runDb(supabase.from('modes').update({ preset: { title: fd.title, bannerUrl: fd.bannerUrl, customText: fd.customText, type: fd.type, perKill: fd.perKill, entryFee: fd.entryFee, totalSlots: fd.totalSlots } }).eq('id', fd.modeId)); md({t:'alert',title:'Success',msg:'Preset saved!'}); } catch(e) { md({t:'err',title:'Error',msg:e.message}); } }} className="px-6 py-4 bg-indigo-100 text-indigo-700 font-black rounded-xl uppercase cursor-pointer hover:bg-indigo-200">Save Preset</button>)}{fd.modeId && d.modes.find(m=>m.id===fd.modeId)?.preset && (<button type="button" onClick={async () => { try { await runDb(supabase.from('modes').update({ preset: null }).eq('id', fd.modeId)); md({t:'alert',title:'Success',msg:'Preset cleared.'}); } catch(e) { md({t:'err',title:'Error',msg:e.message}); } }} className="px-5 py-4 bg-rose-50 text-rose-500 font-black rounded-xl uppercase cursor-pointer hover:bg-rose-100" title="Delete Preset"><Trash2 className="w-5 h-5"/></button>)}</div></form>}
    
    <div className="bg-white rounded-2xl border overflow-hidden shadow-sm">
      <div className="flex bg-slate-50 border-b pr-4">
      ...
      <div className="p-4 space-y-4 bg-slate-100/50">
        {filteredT.length === 0 && <div className="p-8 text-center text-slate-400 font-bold">No {tb} matches found.</div>}
        {Object.keys(groupedByGame).map(gId => {
           const gameName = d.games.find(g=>g.id===gId)?.name || 'Unknown Game'; const isGExp = expG === gId;
           return (
             <div key={gId} className="bg-white border rounded-2xl overflow-hidden shadow-sm">
               <div className="p-4 bg-slate-900 text-white font-black uppercase flex justify-between items-center cursor-pointer hover:bg-slate-800 transition-colors" onClick={()=>{setExpG(isGExp ? null : gId); setExpM(null);}}><div className="flex items-center gap-3"><Gamepad2 className="w-5 h-5 text-blue-500"/> {gameName}</div><ChevronRight className={`w-5 h-5 transition-transform ${isGExp ? 'rotate-90' : ''}`} /></div>
               {isGExp && (
                 <div className="p-4 bg-slate-50 space-y-4">
                   {Object.keys(groupedByGame[gId]).map(mId => {
                     const modeName = d.modes.find(m=>m.id===mId)?.name || 'Unknown Mode'; const isMExp = expM === mId; const matchCount = groupedByGame[gId][mId].length;
                     return (
                       <div key={mId} className="border border-blue-100 rounded-xl bg-white overflow-hidden shadow-sm">
                         <div className="p-3 bg-blue-50/50 text-blue-900 font-bold uppercase flex justify-between items-center cursor-pointer hover:bg-blue-50 transition-colors" onClick={()=>setExpM(isMExp ? null : mId)}><div className="flex items-center gap-2">{modeName} <span className="bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded-full shadow-sm">{matchCount} Matches</span></div><ChevronRight className={`w-4 h-4 text-blue-500 transition-transform ${isMExp ? 'rotate-90' : ''}`} /></div>
                         {isMExp && (
                           <div className="divide-y border-t border-blue-100 bg-white">
                             {groupedByGame[gId][mId].map(t => {
                                if(tb==='history') return <details key={t.id} className="p-5 bg-white group cursor-pointer"><summary className="list-none font-black text-lg uppercase outline-none flex justify-between items-center"><div className="flex items-center gap-3">{t.status==='cancelled'?<XCircle className="text-rose-500 w-5 h-5"/>:<Trophy className="text-emerald-500 w-5 h-5"/>}{t.title} <span className="text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded">{fDate(t.dateTime)}</span></div><div className="flex gap-4">{u.role === 'admin' && <button onClick={(e)=>{e.preventDefault(); md({t:'confirm',title:'Delete Match',msg:'Delete completely from history?',onC:async ()=>{ try { await runDb(supabase.from('tournaments').delete().eq('id', t.id)); md({t:'alert',title:'Success',msg:'Updated successfully'}); } catch(err){ md({t:'err',title:'Error',msg:err.message}); } }})}} className="text-rose-500 bg-rose-50 p-2 rounded cursor-pointer hover:bg-rose-500 hover:text-white"><Trash2 className="w-4 h-4"/></button>}<ChevronRight className="w-5 h-5 text-slate-400 group-open:rotate-90 m-auto"/></div></summary><div className="pt-4 mt-4 border-t space-y-4 cursor-default"><div className="flex gap-4"><div className="bg-slate-50 p-3 rounded border flex-1"><div className="text-[10px] font-black text-slate-400 uppercase">Room ID</div><div className="font-mono font-bold text-lg">{t.roomId||'N/A'}</div></div><div className="bg-slate-50 p-3 rounded border flex-1"><div className="text-[10px] font-black text-slate-400 uppercase">Pass</div><div className="font-mono font-bold text-lg">{t.password||'N/A'}</div></div></div><div className="bg-blue-50 p-3 rounded border border-blue-100 text-sm font-bold">{t.customText||'No custom text.'}</div>{t.status==='result'&&<div className="border rounded-xl overflow-hidden"><table className="w-full text-sm text-left"><thead className="bg-slate-100 text-[10px] uppercase font-black"><tr><th className="p-3">Player</th><th className="p-3">Kills</th><th className="p-3">Earned</th></tr></thead><tbody>{(t.results||[]).map(r=><tr key={r.uid}><td className="p-3 font-bold uppercase">{r.gameName}</td><td className="p-3 font-bold">{r.kills}</td><td className="p-3 font-black text-emerald-600">{r.earned}</td></tr>)}</tbody></table></div>}</div></details>;
                                ...
                                <div className="flex gap-2 flex-wrap bg-slate-50 p-2 rounded-xl border">
                                  {t.status==='upcoming'&&<>
                                    ...
                                    <button onClick={()=>md({t:'room',title:'Set Room & Start',onC: async (r,p)=>{
                                      ...
                                      try {
                                        await runDb(supabase.from('tournaments').update({roomId:r,password:p,status:'ongoing'}).eq('id', t.id));
                                        const targetUids = (t.joinedUsers||[]).map(ju => ju.uid || ju);
                                        if (targetUids.length > 0) { await sendPushNotification({ title: `🎮 ${t.title} — Room is LIVE!`, body: `Room ID: ${r}\nPassword: ${p}`, targetUids, users: d.users, data: { type: 'room_live', roomId: String(r), password: String(p), matchTitle: String(t.title), matchId: String(t.id) } }); }
                                ...
                                <div className="w-full mt-4 bg-slate-50 p-3 rounded-lg border text-xs">
                                  <div className="font-black text-slate-400 uppercase mb-2">Joined Players (Double-Click Name to Flag)</div>
                                  <div className="max-h-48 overflow-y-auto divide-y divide-slate-100 border border-slate-200 rounded-lg">
                                    {(t.joinedUsers||[]).map((ju, i) => {
                                       ...
                                       // 🔥 PERFECT COLOR ROUTING: Friends vs Flags
                                       const sponsor = ju.isFriend ? d.users.find(x=>x.uid===ju.sponsorUid) : null;
                                       const actualUser = d.users.find(x=>x.uid===uid);
                                       const flagUser = ju.isFriend ? sponsor : actualUser; 
                                       
                                       let displayName = actualUser?.gameName || actualUser?.name || 'Player';
                                       let displayClass = "font-black uppercase flex items-center gap-2 cursor-pointer select-none px-3 py-2 rounded-lg transition-colors bg-white hover:bg-slate-50";
                                       
                                       if (ju.isFriend) {
                                          const sponsorName = sponsor ? (sponsor.gameName || sponsor.name) : 'Unknown';
                                          displayName = `${ju.gameName} (friend_${sponsorName})`;
                                          displayClass = "font-black uppercase flex items-center gap-2 cursor-pointer select-none px-3 py-2 rounded-lg transition-colors text-blue-600 bg-blue-50 hover:bg-blue-100";
                                       } 
                                       
                                       if (flagUser?.adminFlag === 'red') {
                                          displayClass = "font-black uppercase flex items-center gap-2 cursor-pointer select-none px-3 py-2 rounded-lg transition-colors text-rose-600 bg-rose-50 border border-rose-200";
                                       } else if (flagUser?.adminFlag === 'violet') {
                                          displayClass = "font-black uppercase flex items-center gap-2 cursor-pointer select-none px-3 py-2 rounded-lg transition-colors text-violet-600 bg-violet-50 border border-violet-200";
                                       }
                                       
                                       return <div key={i} onDoubleClick={()=>setFlagUid(flagUser?.uid)} className="flex justify-between items-center bg-white p-2 border-b last:border-0">
                                           <div className={displayClass}>{displayName}</div>
                                           {ju.joinedAt && <div className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{fDate(ju.joinedAt)}</div>}
                                       </div>;
                                    })}
                                  </div>
                                </div>
...
// 🔥 FINANCIAL MGR: WITHDRAW FILTERS & DOUBLE-CLICK FLAGS 🔥
function FinMgr({ t, d, md, s, u: adminUser }) {
  const [cMsg, setCMsg] = useState('');
  const [flagUid, setFlagUid] = useState(null); // 🔥 State for Double-Click Flag Modal
  const [widFilter, setWidFilter] = useState('all'); // 🔥 State for Withdraw Filters: 'all', 'first', 'repeated'
      ...
      try {
        if(st==='approve') {
          if(t==='deposit') {
            ...
            if (uu) {
              await runDb(supabase.from('users').update({ balance: Number(uu.balance || 0) + Number(tx.amount), depositBalance: Number(uu.depositBalance || 0) + Number(tx.amount) }).eq('uid', tx.uid));
            }
            if (uu && uu.referredBy && s.referralBonusPercent > 0) {
              ...
              if (refUser) {
                ...
                await runDb(supabase.from('users').update({ referralBalance: Number(refUser.referralBalance||0) + bonus }).eq('uid', refUser.uid));
              }
            }
          }
          await runDb(supabase.from('transactions').update({status:'completed'}).eq('id', tx.id));
        } else {
          if(t==='withdraw') {
            if (tx.type==='referral_withdraw_pending') {
              const uu = d.users.find(x=>x.uid===tx.uid);
              if(uu) await runDb(supabase.from('users').update({ referralBalance: Number(uu.referralBalance||0) + Math.abs(tx.amount) }).eq('uid', tx.uid));
            } else {
              const uu = d.users.find(x=>x.uid===tx.uid);
              if (uu) {
                await runDb(supabase.from('users').update({ balance: Number(uu.balance || 0) + Math.abs(tx.amount), winningBalance: Number(uu.winningBalance || 0) + Math.abs(tx.amount) }).eq('uid', tx.uid));
              }
            }
          }
          await runDb(supabase.from('transactions').update({status:'failed'}).eq('id', tx.id));
        }
  ...
  // 🔥 FILTER LOGIC FOR PENDING TRANSACTIONS 🔥
  let p = d.transactions.filter(x=> (t==='withdraw' ? (x.type==='withdraw_pending' || x.type==='referral_withdraw_pending') : x.type===`${t}_pending`) && x.status==='pending');

  if (t === 'withdraw' && widFilter !== 'all') {
    p = p.filter(x => {
      const compWids = d.transactions.filter(tx => tx.uid === x.uid && (tx.type === 'withdraw_pending' || tx.type === 'referral_withdraw_pending') && tx.status === 'completed').length;
      if (widFilter === 'first') return compWids === 0;
      if (widFilter === 'repeated') return compWids > 0;
      return true;
    });
  }
  ...
  return <div className="bg-white p-6 rounded-2xl border relative">
  {cMsg && <div className="fixed top-5 left-1/2 -translate-x-1/2 bg-emerald-500 text-white px-4 py-2 rounded-xl z-50 font-black shadow-lg animate-in fade-in zoom-in">{cMsg}</div>}
  ...
  {flagUid && (
    <div className="fixed inset-0 bg-slate-900/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-xs text-center animate-in zoom-in-95">
        <h3 className="font-black uppercase tracking-widest mb-4 text-slate-800">Flag User</h3>
        <div className="space-y-3">
          <button onClick={async ()=>{ await runDb(supabase.from('users').update({adminFlag:'red'}).eq('uid',flagUid)); setFlagUid(null); md({t:'alert',title:'Flagged',msg:'User marked as Red.'}); }} className="w-full py-4 bg-rose-600 text-white font-black rounded-xl uppercase tracking-widest shadow-md hover:bg-rose-700 cursor-pointer">Red Flag (Warning)</button>
          <button onClick={async ()=>{ await runDb(supabase.from('users').update({adminFlag:'violet'}).eq('uid',flagUid)); setFlagUid(null); md({t:'alert',title:'Flagged',msg:'User marked as Violet.'}); }} className="w-full py-4 bg-violet-600 text-white font-black rounded-xl uppercase tracking-widest shadow-md hover:bg-violet-700 cursor-pointer">Violet Flag (VIP)</button>
          <button onClick={async ()=>{ await runDb(supabase.from('users').update({adminFlag:null}).eq('uid',flagUid)); setFlagUid(null); md({t:'alert',title:'Cleared',msg:'Flags removed.'}); }} className="w-full py-4 bg-slate-100 text-slate-600 font-black rounded-xl uppercase tracking-widest hover:bg-slate-200 cursor-pointer">Remove Flag</button>
        </div>
        <button onClick={()=>setFlagUid(null)} className="mt-5 text-xs font-bold text-slate-400 uppercase tracking-widest cursor-pointer">Cancel</button>
      </div>
  ...
  <h2 className="text-xl font-black uppercase mb-6 flex items-center gap-2"><Clock className="text-amber-500"/> Pending {t}s ({p.length})</h2>

  {/* 🔥 NEW FEATURE: WITHDRAWAL FILTERS 🔥 */}
  {t === 'withdraw' && (
    <div className="flex bg-slate-100 rounded-xl p-1 mb-6 border border-slate-200">
       <button onClick={()=>setWidFilter('all')} className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all cursor-pointer select-none ${widFilter==='all'?'bg-white text-blue-600 shadow-sm':'text-slate-500'}`}>All Requests</button>
       <button onClick={()=>setWidFilter('first')} className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all cursor-pointer select-none ${widFilter==='first'?'bg-white text-emerald-600 shadow-sm':'text-slate-500'}`}>First Time</button>
       <button onClick={()=>setWidFilter('repeated')} className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all cursor-pointer select-none ${widFilter==='repeated'?'bg-white text-amber-600 shadow-sm':'text-slate-500'}`}>Repeated</button>
    </div>
  )}
  ...
  {p.length===0?<div className="p-8 text-center font-bold text-slate-400 bg-slate-50 rounded-xl">All caught up!</div>:p.map(x=>{
    ...
    // 🔥 COLOR ROUTING FOR ADMIN FLAGS 🔥
    const fCol = uu?.adminFlag === 'red' ? 'text-rose-600 bg-rose-100 border-rose-300' : uu?.adminFlag === 'violet' ? 'text-violet-600 bg-violet-100 border-violet-300' : 'text-blue-600 bg-blue-50 border-transparent';
    ...
    return <div key={x.id} className="p-4 bg-amber-50/50 border border-amber-200 rounded-xl flex justify-between items-start">
      <div className="flex-1 pr-4">
        <div className="font-black text-3xl mb-2">{Math.abs(x.amount)}</div>
        
        {/* 🔥 THE DOUBLE CLICK WRAPPER 🔥 */}
        <div onDoubleClick={()=>setFlagUid(x.uid)} className={`text-sm font-black uppercase inline-block px-3 py-1 rounded-lg cursor-pointer select-none border transition-colors ${fCol}`}>
           {uu.name || 'Unknown User'} 
           {x.type==='referral_withdraw_pending' && <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-[9px] ml-2 tracking-widest border-none">REFERRAL</span>}
           {t === 'withdraw' && totalWidCount === 0 && <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[9px] ml-2 tracking-widest border-none">FIRST</span>}
        </div>
        
        <div className="text-xs font-bold text-slate-700 mt-2 flex items-center gap-2">{x.description} {upi && <button onClick={()=>cT(upi)} className="bg-blue-100 text-blue-700 p-1 rounded hover:bg-blue-200 cursor-pointer"><Copy className="w-3 h-3"/></button>}</div>
      ...
      <div className="flex flex-col gap-2 shrink-0">
        <button onClick={()=>act(x,'reject')} className="px-5 py-3 bg-white text-rose-600 font-black uppercase text-xs rounded-lg border border-rose-200 cursor-pointer hover:bg-rose-50 active:scale-95 transition-transform">Reject</button>
        <button onClick={()=>act(x,'approve')} className="px-5 py-3 bg-emerald-500 text-white font-black uppercase text-xs rounded-lg cursor-pointer hover:bg-emerald-600 active:scale-95 transition-transform">Approve</button>
      </div>
    </div>
  })}</div>
  ...
  <table className="w-full text-sm text-left"><thead className="bg-slate-100 text-[10px] uppercase font-black"><tr><th className="p-3">User</th><th className="p-3">Details</th><th className="p-3">Amount</th><th className="p-3">Status</th><th className="p-3">Date</th><th className="p-3">Action</th></tr></thead><tbody className="divide-y">{hT.map(x=>{
    const upi = x.description?.includes('to ') ? x.description.split('to ')[1] : '';
    const histU = d.users.find(u=>u.uid===x.uid) || {};
    const histCol = histU?.adminFlag === 'red' ? 'text-rose-600 bg-rose-50 border border-rose-200' : histU?.adminFlag === 'violet' ? 'text-violet-600 bg-violet-50 border border-violet-200' : '';

    return <tr key={x.id}>
    <td className="p-3 font-bold">
      {/* 🔥 DOUBLE CLICK FLAG IN HISTORY TABLE TOO 🔥 */}
      <span onDoubleClick={()=>setFlagUid(x.uid)} className={`cursor-pointer select-none px-2 py-1 rounded transition-colors ${histCol}`}>
        {histU.name || 'Unknown'} 
      </span>
      {x.type==='referral_withdraw_pending'&&<span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-[8px] ml-2">REF</span>}
    </td>
    ...
    <td className="p-3 font-black">{Math.abs(x.amount)}</td><td className="p-3"><span className={`text-[9px] px-2 py-1 uppercase font-black rounded ${x.status==='completed'?'bg-emerald-100 text-emerald-700':'bg-rose-100 text-rose-700'}`}>{x.status}</span></td><td className="p-3 text-[11px] font-bold text-slate-500">{fDate(x.date)}</td><td className="p-3">
      {adminUser.role === 'admin' && <button onClick={()=> md({t:'confirm',title:'Delete History',msg:'Remove from panel?',onC:async()=>{try{await runDb(supabase.from('transactions').update({adminDeleted:true}).eq('id', x.id)); md({t:'alert',title:'Success',msg:'Updated successfully'})}catch(err){md({t:'err',title:'Error',msg:err.message})}}}) } className="bg-rose-100 text-rose-600 px-3 py-1 rounded text-xs font-black cursor-pointer">DELETE</button>}
    </td></tr>
  })}</tbody></table>
  </div>;
}
...
function StaffMgr({ d, md, u }) {
  ...
  return <div className="bg-white p-6 rounded-2xl border"><div className="flex justify-between items-center mb-6"><h2 className="text-xl font-black uppercase">Staff Panel</h2><button onClick={()=>md({t:'staff',title:'Add Staff',onC:(e,p)=>{const uu=d.users.find(x=>x.email===e); if(uu){ if(uu.role==='admin') return md({t:'err',title:'Error',msg:'Cannot modify Super Admin.'}); runDb(supabase.from('users').update({role:'staff',permissions:p}).eq('uid', uu.uid)); } else md({t:'err',title:'Error',msg:'User not found.'});}})} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-black text-xs uppercase cursor-pointer"><Plus className="w-4 h-4 inline"/> Add Staff</button></div><div className="space-y-3">{sList.length===0&&<div className="p-6 text-center text-slate-400 font-bold bg-slate-50 rounded-xl">No staff.</div>}{sList.map(s=><div key={s.uid} className="p-5 border rounded-xl flex justify-between items-center bg-white shadow-sm"><div><div className="font-black text-2xl uppercase">{s.name}</div><div className="text-sm font-bold text-slate-500 mb-2">{s.email}</div><div className="flex gap-1">{s.permissions.map(p=><span key={p} className="text-[9px] bg-slate-100 text-slate-600 px-2 py-1 rounded border uppercase font-bold">{p}</span>)}</div></div><div className="flex gap-2"><button onClick={()=>{const po={dashboard:0,users:0,games:0,tournaments:0,finance:0,staff:0,settings:0}; s.permissions.forEach(x=>po[x]=1); md({t:'staff',title:'Edit Staff',ed:true,d1:s.email,dp:po,onC:(e,p)=>runDb(supabase.from('users').update({permissions:p}).eq('uid', s.uid))});}} className="px-4 py-2 bg-slate-100 font-black text-xs uppercase rounded cursor-pointer"><Edit className="w-4 h-4"/></button><button onClick={()=>md({t:'confirm',title:'Remove Staff',msg:'Demote to regular user?',onC:()=>runDb(supabase.from('users').update({role:'user',permissions:[]}).eq('uid', s.uid))})} className="px-4 py-2 bg-rose-50 text-rose-600 font-black text-xs uppercase rounded border border-rose-100 cursor-pointer"><Trash2 className="w-4 h-4"/></button></div></div>)}</div></div>;
}
...
function SetMgr({ s, md, u: adminUser }) {
  ...
  const save = async (newState = null) => {
    setSv(true);
    try {
      await runDb(supabase.from('settings').upsert({ id: 'global', data: newState || ls }));
