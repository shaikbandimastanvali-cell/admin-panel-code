...
const copyText = async (text, successCallback) => {
  try {
    if (navigator.clipboard && window.isSecureContext) { await navigator.clipboard.writeText(String(text)); successCallback?.(); return; }
    const textarea = document.createElement("textarea"); textarea.value = String(text); textarea.style.position = "fixed"; textarea.style.left = "-999999px"; document.body.appendChild(textarea); textarea.focus(); textarea.select(); const success = document.execCommand("copy"); document.body.removeChild(textarea); if (success) successCallback?.(); else alert("Copy failed");
...
export default function AdminApp() {
  ...
  const [data, setData] = useState({ users: [], games: [], modes: [], tournaments: [], transactions: [], bannedDevices: [], messages: [], staff: [] });
  ...
  useEffect(() => {
    ...
    document.addEventListener('contextmenu', handleContext); document.addEventListener('keydown', handleKey);
  ...
  useEffect(() => {
    if (!fbUser) return;
    getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'global')).then(d => { if (d.exists()) setSettings(p => ({...p, ...d.data()})); });
  }, [fbUser]);
  ...
  useEffect(() => {
    ...
    if (sid) {
      getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'users', sid)).then(snap => {
        if (snap.exists() && (snap.data().role === 'admin' || snap.data().role === 'staff')) setAdmin({ id: snap.id, ...snap.data() });
        else { localStorage.removeItem('admin_uid'); setAdmin(null); }
      });
    }
  }, []);
  ...
  const login = async (e) => {
    ...
    if (!snap.empty) {
      ...
      if (u.role === 'admin' || u.role === 'staff') { localStorage.setItem('admin_uid', u.uid); setAdmin(u); return; }
    }
...
function SectionHeader({ title, onRefresh, loading }) {
  return (
    <div className="flex justify-between items-center bg-white p-4 rounded-2xl border shadow-sm mb-6">
      <h2 className="font-black text-xl uppercase tracking-widest text-slate-800">{title}</h2>
      <button onClick={onRefresh} disabled={loading} className="bg-blue-600 text-white px-5 py-2 rounded-xl font-black uppercase text-xs disabled:opacity-50 flex items-center gap-2 cursor-pointer shadow-md hover:bg-blue-700 transition-colors">
...
function AdminLayout({ u, data, setData, sets, out }) {
  ...
  const vMap = {
    ...
    withdraws: <FinMgr t="withdraw" md={setMd} s={sets} u={u} cache={data} setCache={setData}/>, 
    luckywheel: <LuckyWheelMgr md={setMd} u={u}/>,
    messages: <MessageMgr md={setMd} u={u} cache={data} setCache={setData}/>, 
    staff: <StaffMgr md={setMd} u={u} cache={data} setCache={setData}/>, 
    deviceBans: <DeviceBanMgr md={setMd} u={u} cache={data} setCache={setData}/>, 
...
function UniModal({ m, sm }) {
  ...
  const [prm, setP] = useState(m.dp||{dashboard:1,users:0,games:0,tournaments:0,finance:0,luckywheel:0});
  const [cd, setCd] = useState(m.reqTimer ? 5 : 0);
  const [searchRes, setSearchRes] = useState([]);

  useEffect(() => { if (cd > 0) { const timer = setTimeout(() => setCd(cd - 1), 1000); return () => clearTimeout(timer); } }, [cd]);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 z-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95"><div className={`p-4 font-black uppercase text-white ${m.t==='err'?'bg-rose-500':m.t==='confirm'?'bg-amber-500':'bg-blue-600'}`}>{m.title}</div><div className="p-6 space-y-4 font-medium text-slate-700">{m.msg&&<p>{m.msg}</p>}
      ...
      {m.t==='staff'&&<><input type="email" placeholder="User Email" value={i1} disabled={m.ed} onChange={e=>s1(e.target.value)} className="w-full p-3 border rounded-lg font-bold disabled:bg-slate-100"/><div className="grid grid-cols-2 gap-2">{['dashboard','users','games','tournaments','finance','luckywheel'].map(k=><button key={k} onClick={()=>setP({...prm,[k]:!prm[k]})} className={`p-2 rounded border text-xs font-bold uppercase flex justify-between cursor-pointer ${prm[k]?'bg-blue-50 border-blue-500 text-blue-700':''}`}>{k} {prm[k]&&<CheckCircle2 className="w-4 h-4"/>}</button>)}</div></>}
...
function Dash({ s }) {
  ...
  return <div className="space-y-6">
    <SectionHeader title="Global Statistics" onRefresh={handleRecalculate} loading={calculating} />
    <div className="text-xs font-bold text-slate-500 -mt-4 mb-4 ml-2">Last updated: {stats.lastUpdated ? fDate(stats.lastUpdated) : 'Never'}</div>
    ...
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <div className={`p-6 rounded-2xl border flex justify-between bg-blue-50 border-blue-200`}>
        <div className={`font-black text-blue-800`}><div className={`text-xs uppercase tracking-widest text-blue-600`}>Cost Saving Mode Active</div><div className="text-sm mt-2 font-medium opacity-80 max-w-[200px]">Dashboard uses minimal reads. Click "Refresh Data" to calculate totals.</div></div>
...
function UserMgr({ md, s, u: adminUser, cache, setCache }) {
  ...
  const [srt, setSrt] = useState('joinedDate');
  ...
  useEffect(() => {
    ...
    getDocs(query(baseRef('users'), where('referredBy', '==', su.uid))).then(snap => {
  ...
  if (su) {
    ...
    return (
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div><h3 className="font-black text-xl mb-4">Profile</h3><div className="bg-slate-50 p-5 rounded-xl border space-y-3"><div><div className="text-[10px] font-bold text-slate-400 uppercase">Name & IGN</div><div onClick={()=>cT(su.name)} className="font-black text-lg cursor-pointer hover:text-blue-600 transition-colors" title="Click to copy">{su.name} <span className="text-blue-600 text-sm">({su.gameName||'N/A'})</span></div></div><div><div className="text-[10px] font-bold text-slate-400 uppercase">Game UID & Account Email</div><div className="font-mono font-bold text-sm cursor-pointer hover:text-blue-600 transition-colors" title="Click to copy" onClick={()=>cT(`${su.gameUid || su.uid} | ${su.email}`)}>Game UID: {su.gameUid || 'N/A'} <br/> <span className="text-slate-400 text-xs mt-1 block">Login: {su.email}</span></div></div><div><div className="text-[10px] font-bold text-slate-400 uppercase">Password</div><div className="flex gap-2 items-center font-mono font-bold bg-white p-2 rounded border w-max">{sp?su.password:'••••••••'} <button onClick={()=>setSp(!sp)} className="text-blue-600 text-xs ml-2 underline cursor-pointer">{sp?'Hide':'Show'}</button></div></div>
          <div><div className="text-[10px] font-bold text-slate-400 uppercase">Push Notifications</div><div className={`text-xs font-black px-2 py-1 rounded w-max ${su.fcmToken ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-600'}`}>{su.fcmToken ? '✓ FCM Token Registered' : '✗ No FCM Token'}</div></div>
        ...
        </div><button onClick={async ()=>{ if(!su.deviceId) return md({t:'err',title:'Error',msg:"No device ID recorded for this user yet."}); try { await setDoc(doc(db,'artifacts',appId,'public','data','bannedDevices',su.uid),{deviceId:su.deviceId, bannedAt:new Date().toISOString()}); md({t:'alert',title:'Success',msg:'Hardware Device Banned Successfully'}); } catch(err){ md({t:'err',title:'Error',msg:err.message}); } }} className="w-full mt-2 py-3 rounded-lg font-black uppercase text-xs bg-slate-900 text-white cursor-pointer hover:bg-slate-800 transition-colors">Ban Hardware Device</button>
        </div>
        ...
        <div className="max-h-60 overflow-y-auto space-y-2">
          {invited.map(i=>(
            <div key={i.uid} className="bg-white p-3 border rounded flex flex-col gap-1"><div className="flex justify-between items-center"><div className="font-black text-sm uppercase">{i.name}</div><div className="text-emerald-600 font-black text-xs text-right">Deposited: {i.totalDeposited || 0} {s.currencySymbol}</div></div><div className="text-[10px] font-bold text-slate-500">{i.email} | Game UID: {i.gameUid || 'N/A'}</div><div className="text-[9px] text-slate-400 font-bold uppercase">Joined: {fDate(i.joinedDate)}</div></div>
          ))}
        </div></div></div>
      </div></div>
    );
...
function FinMgr({ t, md, s, u: adminUser, cache, setCache }) {
  ...
  const pending = cache[`${t}_pending`] || [];
  const history = cache[`${t}_history`] || [];
  ...
  <div className="flex justify-between items-center mb-6 border-b pb-4">
    ...
    {t === 'withdraw' && <div className="flex gap-2">
       ...
       <button onClick={()=>setWTab('repeated')} className={`px-4 py-2 text-xs font-black uppercase rounded-lg cursor-pointer ${wTab==='repeated'?'bg-purple-600 text-white':'bg-slate-100 text-slate-500'}`}>Repeated</button>
    </div>}
  </div>
    ...
    <div className="p-4 bg-amber-50/50 border border-amber-200 rounded-xl flex justify-between items-start shadow-sm">
      <div className="flex-1 pr-4">
        ...
        <div className="text-sm font-black text-blue-600 uppercase">
          {uData?.name || 'Unknown User'} {x.type==='referral_withdraw_pending'&&<span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-[9px] ml-2 tracking-widest align-middle">REFERRAL WITHDRAW</span>}
        </div>
        <div className="text-xs font-bold text-slate-500 mt-2 flex items-center gap-2 flex-wrap">
          <span className="bg-slate-200 text-slate-800 px-2 py-1 rounded font-black">GAME UID: {uData?.gameUid || 'N/A'}</span> 
        ...
        <div className="bg-white border border-amber-200 rounded-lg p-3 grid grid-cols-2 gap-y-2 gap-x-4 text-[10px] font-black uppercase tracking-widest text-slate-600 shadow-inner">
          ...
          <div className="col-span-2 pt-1 border-t border-slate-100 mt-1 flex justify-between">
             <span>Games: <span className="text-indigo-600">{uData?.totalMatches || 0}</span></span>
             <span>Refers: <span className="text-purple-600">{uData?.totalRefers || 0}</span></span>
          </div>
        </div>
      </div>
...
function GamesMgr({ md, u, cache, setCache }) {
  const [loading, setLoading] = useState(false);
  const loadData = async () => {
    setLoading(true);
    try {
      const gSnap = await getDocs(baseRef('games')); const mSnap = await getDocs(baseRef('modes'));
      setCache(p=>({...p, games: gSnap.docs.map(d=>({id:d.id, ...d.data()})), modes: mSnap.docs.map(d=>({id:d.id, ...d.data()})) }));
    } catch(e) {}
    setLoading(false);
  };
...
function TourneyMgr({ md, u, s, cache, setCache }) {
  ...
  const sv = async (e) => {
    ...
    if(!fd.modeId)return;
    try {
      if (editId) { await updateDoc(doc(db,'artifacts',appId,'public','data','tournaments',editId), {...fd}); setEditId(null); } 
      else { await addDoc(collection(db,'artifacts',appId,'public','data','tournaments'),{...fd,joinedUsers:[],results:[],createdBy:u.name}); }
      setFo(false); md({t:'alert',title:'Success',msg:'Updated successfully'}); loadData();
    } catch(err) { md({t:'err',title:'Error',msg:err.message}); }
  };
  ...
  const oR = async (t) => { 
     setIsLoadingResults(true); const o={}; 
     try {
         await Promise.all((t.joinedUsers||[]).map(async ju => {
            const id = ju.uid || ju;
            const uSnap = await getDoc(doc(db,'artifacts',appId,'public','data','users',id));
            o[id] = { gameName: uSnap.exists() ? (uSnap.data().gameName || uSnap.data().name) : 'Player', kills: 0, earned: 0, uid: id };
         }));
         setRd(o); setRn(t.customText||''); setRt(t); 
     } catch(e) { md({t:'err', title:'Error', msg: 'Failed to load players.'}); }
        ...
        try {
          ...
          await Promise.all(a.map(async r=>{
            if(r.earned>0||r.kills>0){
              ...
              if (xuSnap.exists()) {
                 const xu = xuSnap.data(); const up={};
                 if(r.earned>0){ up.balance=Number(xu.balance||0)+Number(r.earned); up.winningBalance=Number(xu.winningBalance||0)+Number(r.earned); up.totalWinnings=Number(xu.totalWinnings||0)+Number(r.earned); await addDoc(collection(db,'artifacts',appId,'public','data','transactions'),{uid:r.uid,type:'winnings',amount:r.earned,description:`Won: ${rt.title}`,status:'completed',date:new Date().toISOString()}); }
          ...
          setRt(null); md({t:'alert',title:'Success',msg:'Updated successfully'}); loadData();
...
function DeviceBanMgr({ md, u: adminUser, cache, setCache }) {
  ...
  {bannedDevices.map(b => (
    <div key={b.id} className="p-4 border rounded-xl flex justify-between items-center shadow-sm">
      ...
      {adminUser.role === 'admin' && <button onClick={() => md({t:'confirm',title:'Unban Device?',msg:'Confirm unban',onC:async()=>{try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bannedDevices', b.id)); md({t:'alert',title:'Success',msg:'Device unbanned successfully'}); loadData(); } catch (err) { md({t:'err',title:'Error',msg:err.message}); }}})} className="bg-emerald-100 text-emerald-700 px-4 py-2 rounded-lg font-black text-xs cursor-pointer hover:bg-emerald-200">UNBAN</button>}
    </div>
  ))}</div></div></div>;
}

function MessageMgr({ md, u: adminUser, cache, setCache }) {
  const [loading, setLoading] = useState(false);
  const loadData = async () => {
    setLoading(true);
    ...
    {messages.map(m => (
      <div key={m.id} className="p-5 hover:bg-slate-50 transition-colors flex justify-between items-start gap-4"><div className="flex-1"><div className="font-black text-lg text-slate-800 uppercase">{m.title} {m.targetUids && m.targetUids[0] !== 'all' && <span className="bg-amber-100 text-amber-700 text-[9px] px-2 py-0.5 rounded ml-2 align-middle">PRIVATE MESSAGE</span>}</div><div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 mb-2">{fDate(m.createdAt)}</div><div className="text-sm font-medium text-slate-600 whitespace-pre-wrap">{m.body}</div><div className="text-[10px] text-blue-500 font-bold uppercase mt-3 tracking-widest">Read by: {(m.readBy || []).length} users</div></div><div className="flex gap-2 shrink-0">
        <button onClick={() => md({t:'message', title:'Edit Message', d1: m.title, d2: m.body, onC: async (title, body) => {
          if(!title || !body) return;
          try { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'messages', m.id), { title, body }); md({t:'alert', title:'Success', msg:'Message updated successfully'}); loadData(); } catch(e) { md({t:'err', title:'Error', msg:e.message}); }
        ...
        {adminUser.role === 'admin' && <button onClick={() => md({t:'confirm', title:'Delete Message', msg:'Permanently delete this message for all users?', onC: async () => {
          try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'messages', m.id)); md({t:'alert', title:'Success', msg:'Message deleted'}); loadData(); } catch(e) { md({t:'err', title:'Error', msg:e.message}); }
...
function StaffMgr({ md, u, cache, setCache }) {
  const [loading, setLoading] = useState(false);
  const loadData = async () => {
    setLoading(true);
  ...
  return <div><SectionHeader title="Staff Panel" onRefresh={loadData} loading={loading} /><div className="bg-white p-6 rounded-2xl border shadow-sm"><div className="flex justify-between items-center mb-6"><h2 className="text-xl font-black uppercase">Staff Management</h2><button onClick={()=>md({t:'staff',title:'Add Staff',onC: async (e,p)=>{
...
function SetMgr({ s, md, u: adminUser }) {
  ...
  <div className="bg-slate-50 p-4 rounded-xl border"><div className="flex justify-between items-center mb-4"><h3 className="font-black uppercase text-sm">Support Channels</h3><button onClick={()=>md({t:'social',title:'Add Social Media',onC:(n,u,i)=>{const updated = [...(ls.supportChannels||[]), {name:n,url:u,iconUrl:i}]; sls({...ls, supportChannels: updated}); save({...ls, supportChannels: updated}); }})} className="px-3 py-1 bg-indigo-100 text-indigo-700 font-black text-[10px] uppercase rounded cursor-pointer hover:bg-indigo-200">+ Add</button></div><div className="space-y-2">{(ls.supportChannels||[]).map((c,i)=><div key={i} className="flex justify-between items-center bg-white p-3 border rounded font-bold text-sm shadow-sm"><div>{c.name} <span className="text-[10px] text-slate-400 font-mono block">{c.url}</span></div><button onClick={()=>{const arr=[...(ls.supportChannels||[])]; arr.splice(i,1); sls({...ls,supportChannels:arr}); save({...ls, supportChannels: arr}); }} className="text-rose-500 p-2 cursor-pointer hover:bg-rose-50 rounded"><Trash2 className="w-4 h-4"/></button></div>)}</div></div>
  </div></div>;
}
